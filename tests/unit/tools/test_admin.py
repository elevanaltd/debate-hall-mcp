...
def test_debate_tombstone_redacts_turn(tmp_path: Path) -> None:
    ...
    # Add turns
    debate_turn("test-thread-003", "Wind", "Original Wind", state_dir=tmp_path)
    debate_turn("test-thread-003", "Wall", "Original Wall", state_dir=tmp_path)
    ...
    assert result["thread_id"] == "test-thread-003"
...
def test_debate_tombstone_invalid_index(tmp_path: Path) -> None:
    ...
    debate_turn("test-thread-004", "Wind", "Original", state_dir=tmp_path)
...
def test_debate_tombstone_preserves_hash_chain(tmp_path: Path) -> None:
    ...
    # Add three turns
    debate_turn("test-thread-005", "Wind", "Wind content", state_dir=tmp_path)
    debate_turn("test-thread-005", "Wall", "Wall content", state_dir=tmp_path)
    debate_turn("test-thread-005", "Door", "Door content", state_dir=tmp_path)

    # Store original hashes
    room_before = load_debate_state("test-thread-005", tmp_path)
    ...
    room_after = load_debate_state("test-thread-005", tmp_path)
