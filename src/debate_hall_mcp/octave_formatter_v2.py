...
This replaces the manual string manipulation with the official octave-mcp API,
providing better validation, security, and maintainability.
...
def format_debate_as_octave(
    ...
    try:
        ...
        return _manual_fallback(room, output_mode)


def _manual_fallback(room: DebateRoom, output_mode: OutputMode) -> str:
    """Manual fallback formatter if octave-mcp API is unavailable.

    This is a simplified version that constructs valid OCTAVE manually.
    """
    lines = []
    ...
    # META section
    lines.append("META:")
    lines.append(f'  THREAD_ID::"{room.thread_id}"')
    ...
    # SYNTHESIS
    if room.synthesis:
        synthesis = room.synthesis.replace('"', '\\"')
        lines.append(f'SYNTHESIS::"{synthesis}"')
    else:
        lines.append("SYNTHESIS::null")
    lines.append("")
