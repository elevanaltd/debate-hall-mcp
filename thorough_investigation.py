...
def check_python_info():
    ...
    print(
        f"Virtual Environment: {hasattr(sys, 'real_prefix') or (hasattr(sys, 'base_prefix') and sys.base_prefix != sys.prefix)}"
    )
...
def check_project_structure():
    ...
    dirs_to_check = ["src", "src/debate_hall_mcp", ".venv", ".venv/lib", "tests"]
...
def check_editable_install():
    ...
    try:
        result = subprocess.run(
            [sys.executable, "-m", "pip", "show", "-f", "debate-hall-mcp"],
            capture_output=True,
            text=True,
            check=True,
        )
